...
//Date: 02/07/2019
//
//Update: 10/01/2019  Oliver Chen
//}

warning('off');

// Please enter NATS HOME location here
//NATS_HOME = 'PLEASE_ENTER_NATS_HOME_LOCATION_HERE';
NATS_HOME = '.';

if (~isdir(NATS_HOME)) then
    disp("Directory NATS_HOME does not exist");
else
    chdir(NATS_HOME);
    
    javaclasspath(NATS_HOME + '/dist/nats-standalone.jar');
    javaclasspath(NATS_HOME + '/dist/nats-client.jar');
    javaclasspath(NATS_HOME + '/dist/nats-shared.jar');
    javaclasspath(NATS_HOME + '/dist/json.jar');
    javaclasspath(NATS_HOME + '/dist/commons-logging-1.2.jar');

    SimulationStatus = jimport('com.osi.util.Constants', %f);
    AircraftClearance = jimport('com.osi.util.AircraftClearance', %f);
    
    NATSStandalone = jimport('NATSStandalone', %f);
    ...
    safetyMetricsInterface = natsStandalone.getSafetyMInterface();
    ...
    for count = 1:sampleSize
        ...
        while 1
            ...
            if (simStatus ~= NATS_SIMULATION_STATUS_sleep)
               sleep(1000);
            else
                break;
            end
        ...
        simulationInterface.write_trajectories(strcat(['share/mcSimulation/' trajectoryFile]));
    ...
    for currentTrajectory = 1:trajectoryCount
        ...
        csvFile = mopen(strcat([NATS_HOME '/share/mcSimulation/' aircraftID '-Scilab-Monte-Carlo-Sim-Trajectory_' string(currentTrajectory) '.csv']),'r');
