...
function plotRouteineCalled = PlotGraph(callsign, userTrajectoryCount, platform, NATS_HOME)
    ...
    for currentTrajectory = 1 : trajectoryCount
        ...
        if (platform == 'MATLAB')
            CSVcontents = readtable([NATS_HOME '/share/mcSimulation/' aircraftID '-Monte-Carlo-Sim-Trajectory_' num2str(currentTrajectory) '.csv'],'Delimiter',';','ReadVariableNames',false);
            FlightParameters = table2array(CSVcontents);
        else
            % Graph plotting for visualizing vital flight parameter data 
            S = fileread([NATS_HOME '/share/mcSimulation/' aircraftID '-Octave-Monte-Carlo-Sim-Trajectory_' num2str(currentTrajectory) '.csv']);
